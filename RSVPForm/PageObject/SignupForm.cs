...
namespace RSVPForm.PageObject
{
...

    {
        ...
        public SignupForm(IWebDriver driver)
        {
            _driver = driver;
        }
        private IWebElement PageTitle => _driver.FindElement(By.CssSelector(".hero__content .section-story__preheader"));
        private IWebElement GoSignUpBtn => _driver.FindElement(By.XPath("//a[contains(text(),'Sign Up')]"));
        ...
        public void KitchenProj(string ave1, string ave2, string ave3, string ave4)
        {
            if (ave1 == "$0-50")
            {
                Kitchen50.Click();
        ...
        //What type of projects do you work on most? (Select one)*
        public void ProjectsType(string type1, string type2, string type3)
        {
            if (type1 == "Residential")
            {
                Residential.Click();
            }
            else if (type2 == "Commercial")
            {
                Commercial.Click();
            }
            else if(type3 =="Both")
